...
class ContainerManager:
    ...
    def initialize_connection(self, settings, app) -> None:
        ...
        try:
            ...
            self.client = None
            raise ContainerException("CTFd timed out when connecting to Docker: " + str(e))
        except paramiko.ssh_exception.AuthenticationException as e:
            self.client = None
            raise ContainerException("CTFd had an authentication error when connecting to Docker: " + str(e))
        ...
        if self.expiration_seconds > 0:
            ...
            self.expiration_scheduler.add_job(func=self.kill_expired_containers, args=(app,), trigger="interval", seconds=EXPIRATION_CHECK_INTERVAL)
    ...
    def create_container(self, image: str, port: int, command: str, volumes: str):
        ...
        if self.settings.get("container_maxmemory"):
            try:
                ...
                ContainerException("Configured container memory limit must be an integer")
        ...
        try:
            return self.client.containers.run(
                image,
                ports={str(port): str(random.randint(10000, 65536))}, # TODO: Fix this port randomization
                privileged=True,
                tty=True,
                stdin_open=True,
                detach=True,
    ...
    @run_command
    def get_container_health(self, container_id: str) -> "str|None":
        try:
            return self.client.containers.get(container_id).attrs["State"]["Health"]["Status"]
        except (KeyError, IndexError) as e:
            return None
